...
## CFL-95 Calculator Design Tweaks (joint sign-off complete, merged)

Reviewed as an interactive Artifact mockup with Gavi across many rounds (11+ iterations covering desktop and mobile, https://claude.ai/code/artifact/9c378fcb-4cbe-47e6-a59c-1bf49081210d) before any code changed, matching this project's established practice for layout changes (see the CFL-25 mockup precedent below). Gavi approved the mockup; this revised the Scientific-controls grid position and introduced new interaction patterns (a slider toggle, icon-only History/Export controls, a non-reflowing side panel) that design.md's AI Rules require joint sign-off for before permanent/final status. **Joint sign-off completed 2026-07-31**: Eldad approved PR #63 (after one real review round covering four findings, two genuine and fixed, two flagged as not reproducing) and it merged to `main`, verified live in production and confirmed by Gavi. No longer pending.
...
- **Export consolidation (2026-07-31, post-merge follow-up):** the header's Export icon now opens a small menu with two items, "Export Logs" and "Export Telemetry", rather than the icon covering only the real submission log while CFL-91's "Export Telemetry" sat as a separate plain-text button below the calculator. The two exports produce genuinely different files (the real log interleaves `BUTTON_PRESS` with `CALCULATION_SUCCESS`/`CALCULATION_ERROR` evidence events; telemetry omits the calculation events and adds computed metrics) and are **not** merged into one file -- only the trigger UI is consolidated into one icon. Both export status messages now auto-clear after 4 seconds instead of persisting indefinitely (a pre-existing bug in both original controls, not new to this consolidation).
