...
- **Memory indicator** moves from its own row inside the Scientific-controls grid to the display's top-left, mirroring the angle-mode indicator (which stays bottom-left, alongside the current value). Top row: memory indicator (left), previous expression / error message (right). Bottom row: angle-mode indicator (left), current value (right).
