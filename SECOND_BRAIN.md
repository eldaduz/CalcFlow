...
- Human owner: Gavi
- AI used: Claude
- [PR #66](https://github.com/eldaduz/CalcFlow/pull/66) merged and QA'd this update — see the dedicated PR #66 entry below for the full sequence (unintended merge, post-merge QA, Production deployment gap still open).
...
- **Gavi — CFL-28 (Log Export and Submission Evidence): Ready for Deployment.** Its `logs/calcflow-submission-log.json` is exported and committed ([PR #60](https://github.com/eldaduz/CalcFlow/pull/60), merged, Eldad-approved). Whether that approval counts as CFL-28's own required second-teammate log review (same judgment call as the CFL-33/88 precedent) is still open. Status corrected by Eldad 2026-08-02 (Code Review → briefly Done → Ready for Deployment, see PR #66 entry below for why). **CFL-78** (its child Task) tracks the same status.
- **Gavi — Export control consolidation (2026-07-31, follow-up, not its own Jira item):** after CFL-95 moved Export Logs into a header icon, CFL-91's separate "Export Telemetry" button was left as a stray plain-text control below the calculator, and both export status messages were found to never auto-clear. [PR #66](https://github.com/eldaduz/CalcFlow/pull/66) consolidates both exports into one header icon with a small menu, and fixes the status-message bug — logged as comments on CFL-91 and CFL-28 rather than a new Jira Bug, per Gavi's explicit call. **Merged 2026-08-02, but unintentionally** (Eldad meant to approve only; QA was meant to happen before merge) — see the dedicated PR #66 entry below for the full sequence, post-merge QA results, and an open Production deployment gap.
...
1. Eldad: investigating why Vercel Production hasn't deployed commit `2ddbab7` (PR #66's merge) — GitHub shows zero deployment attempts for that SHA, needs the Vercel dashboard to diagnose further (see PR #66 entry below); this blocks the Production smoke test needed to close out PR #66/CFL-28
2. Gavi: still needs to make the call on whether Eldad's PR #60 approval counts as CFL-28's required second-teammate log review
3. Eldad: CFL-31 (execute release) — comes after everything above lands, per the release sequence
...
- **Open: Production has not deployed this merge.** https://calc-flow-fawn.vercel.app/ still serves the pre-#66 build (`last-modified: 2026-07-31T15:04:44Z`, matches PR #63/CFL-95). GitHub's deployment API confirms zero deployment attempts exist for `2ddbab7` (no failure recorded — it simply never triggered; most recent Production deployment on record is still `deaef58`). Root cause needs the Vercel dashboard (Git integration settings, webhook delivery log) — Gavi has no Vercel access, Eldad does and is investigating.
- Next safe action: Eldad to diagnose the Vercel deploy gap. Once Production reflects `2ddbab7`, run the Production smoke test before this can be considered fully closed.

## Latest Handoff

- Current work: v0.6.0, CFL-30, and all of CFL-90/91/92/93/94/95 are **Done**, merged, and verified live in production (dry-run QA + post-merge QA + production smoke test, all confirmed by Gavi). CFL-33/CFL-88 confirmed Done live. **PR #66 merged 2026-08-02 (unintentionally, per Eldad — see its dedicated entry above) and passed post-merge QA on `main`, but Production has not yet deployed it** — GitHub shows zero deployment attempts for the merge commit; Eldad is investigating via the Vercel dashboard (Gavi has no access there). CFL-28/CFL-78 are Ready for Deployment; still open whether Eldad's PR #60 approval counts as the required second-teammate log review.
- Branch cleanup, two passes 2026-07-31: 39 stale branches deleted in the first pass (33 from already-merged historical PRs, 4 merged this session, 2 from PRs closed as superseded — #59, #61), then 4 more (3 local, 1 remote) after #60/#63/#64 merged. Only `main` and the currently-active PR branch remain at any given time — check `git branch --list` and `gh`/API branch list before assuming this stays true. `fix/export-menu-consolidation` (PR #66's source branch) intentionally left undeleted for now, in case it's needed while the unintended-merge situation is still being sorted out with Eldad.
...
- Next safe action: Eldad to diagnose the Vercel deploy gap for `2ddbab7`; once Production reflects it, run the Production smoke test. Separately, Gavi's call on CFL-28/CFL-78's second-teammate review question.
