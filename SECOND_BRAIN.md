...
- Jira status: Ready for Deployment. Done still requires an explicit, confirmed production smoke test (Section 12/13) — now technically possible for the first time since CFL-13 (merged below) gives the app an actual UI, but not yet performed/confirmed. Formal deployment/smoke-test tracking (CFL-30/CFL-82) remains in Backlog; holding per Gavi's "just wait" on that gap.
...
- Current Feature: CFL-13 — Basic Calculator Interaction — **merged, QA'd, Ready for Deployment**
- Branch: `feature/CFL-13-basic-calculator-interaction`, merged via PR #2 (Eldad approved 2026-07-26 10:23 UTC, merge commit `5e67c3c`).
- Pull Request: **PR #2 merged** — https://github.com/eldaduz/CalcFlow/pull/2, `feature/CFL-13-basic-calculator-interaction` → `main`.
...
- Post-rebase fix: CFL-12's review fix (commit `0320094`) removed all display rounding from `arithmetic.js`, explicitly deferring it to CFL-13/CFL-14 (raw results like `0.1 + 0.2 === 0.30000000000000004` now pass through unrounded). `calculatorEngine.js`'s `formatOperand` rounds for display via `toPrecision(12)` instead — chosen specifically because, unlike scaling by `10**n` (the approach CFL-12 just removed for this exact reason), it cannot turn an already-finite result into `Infinity`.
- PR #2 review round, both addressed in commit `926d5ff`:
  1. **Spec gap, blocking ("please fix before approval"):** `toggleSign` operated on the frozen first-operand display echo while `awaitingOperand` was true, so `8 + ± 3 =` displayed `-8` but the next digit typed silently discarded it, evaluating as `8 + 3 = 11`. Fixed by making sign toggle inert in that state, mirroring delete's existing behavior there.
...
- Design judgment calls CFL-13 originally flagged are resolved by Gavi/Eldad — see "Design Decision Resolution" below. No code changes were needed as a result (both matched what was already implemented). Note: design.md's item 4 ("0 button double width") can now be safely removed from "Open Design Decisions" whenever convenient, since both CFL-12 and CFL-13 have landed — not done yet, not requested.
- QA (post-merge, on `main` at `5e67c3c`, full evidence on the Jira issue): real pipeline (`npm ci`/lint/format/test/coverage/build) all green, 52/52 tests, 91.86% coverage, regression clean against CFL-9/10/12. All acceptance criteria walked through fresh in a real browser against the actual merged code (integers/decimals, chaining, clear, delete, sign toggle incl. the regression fix, float-display rounding, error + recovery without reload), plus edge/negative cases (double-decimal prevention, delete-to-zero boundary, operator swap before operand, inert stray keypress). No defects found, no Bug filed.
- Next required action: none for CFL-13 itself. A genuine production smoke test (through the now-live UI) is possible for the first time but not yet run — holding per Gavi's "just wait" on the broader CFL-30/CFL-82 deployment/smoke-test Jira gap. CFL-14 planning can resume whenever Gavi wants to proceed.
- Human approval required: Yes, at the standard PROJECT_PLAN control points (production smoke test confirmation before Done)
