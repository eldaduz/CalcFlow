...
- Blocker: GitHub CLI (`gh`) is not installed, so the required publish workflow cannot push the branch or open the primary PR.
- Next safe action: install and authenticate `gh`, then push the Feature branch, open the primary PR, move CFL-17/CFL-55/CFL-56 to Code Review, and request Gavi's review.
