...
- Human owner: Gavi
- AI used: Claude
- **Note on this session's own process gap:** this file was not updated at the PR-opened milestone for CFL-90/91/93/94 until asked directly (see the entry below); this update landed alongside CFL-95's PR, and this specific revision reconciles the three-way PR pileup that briefly resulted (see "PR consolidation" below) — same lesson restated because it recurred twice.

## Current Release

- **Gavi — v0.6.0 (Complete User Experience) is fully Done**: CFL-24, CFL-25, CFL-26 (CFL-73/74) all Done, merged, deployed, smoke-tested. Of the original three v1.0.0 Features: **CFL-30 (Vercel Deployment) is Done.** **CFL-33 (License Reporting) and CFL-88 are Done, verified live** — Eldad's PR #50 review counted as the required second-teammate license sign-off. **CFL-28 (Log Export) is still Code Review**: it's missing its actual committed `logs/calcflow-submission-log.json` — a ready copy of that file exists in open [PR #60](https://github.com/eldaduz/CalcFlow/pull/60), not yet merged.
- **Gavi — Novelty Telemetry (Epic CFL-92, plus CFL-90/91 under CFL-6/CFL-28):** CFL-90, CFL-91, CFL-93, CFL-94 are all **merged to `main`** — [PR #56](https://github.com/eldaduz/CalcFlow/pull/56) (CFL-90), [PR #57](https://github.com/eldaduz/CalcFlow/pull/57) (CFL-91), [PR #58](https://github.com/eldaduz/CalcFlow/pull/58) (CFL-93/94). **CFL-92 (the Epic) corrected 2026-07-31 from Backlog to In Progress** — it doesn't auto-roll-up from CFL-93/94. Jira status for CFL-90/91/93/94 themselves and CFL-92 not yet re-verified against this merge as of this doc update — check live before assuming Done. See the dedicated entry below for full scope detail.
- **Gavi — CFL-95 (Calculator Design Tweaks):** code-complete, Code Review, [PR #63](https://github.com/eldaduz/CalcFlow/pull/63), now including two follow-up fixes (memory indicator hidden in Basic mode; History panel spans the calculator's full height on desktop) pushed 2026-07-31. Reviewed as an interactive Artifact mockup across 11+ rounds before any code changed; **pending Eldad's joint sign-off per design.md's Design Control rule** (same status as CFL-25's still-open base-keypad decision) since it revises the Scientific-grid position and introduces new interaction patterns. See its own entry below.
...
## PR consolidation (2026-07-31)

A three-way pileup briefly existed — [PR #59](https://github.com/eldaduz/CalcFlow/pull/59), [PR #60](https://github.com/eldaduz/CalcFlow/pull/60), and [PR #61](https://github.com/eldaduz/CalcFlow/pull/61) were all touching this exact file at once, none merged, none reviewed. Resolved as follows:

- **#59** (catch-up for CFL-90/91/93/94 + CFL-5/CFL-7 epic corrections): fully superseded — every fact it carries (the detailed Novelty Telemetry entry below, and the Approved Operating Decisions bullet about epic drift) is folded into this revision. Closed as superseded rather than merged, to avoid two overlapping edits to the same sections landing separately.
- **#60** (adds the real `logs/calcflow-submission-log.json` CFL-28 needs): **merged.** This carried real, non-doc content (the actual evidence file), not just a documentation claim, and landed on `main` ahead of this revision (which needed a rebase over it as a result).
- **#61** (CFL-33/CFL-88 Done claim): fully superseded — the claim was independently verified live as part of this update (see the CFL-30/CFL-28/CFL-33 entry above) and is now the record of truth here. Closed as superseded.

Same-file collisions across this many simultaneous docs PRs have now happened twice (see the PR #52 vs #53 incident below) — the recurring cause is landing a docs update in its own PR that ends up outliving its subject PR. Going forward, prefer folding a SECOND_BRAIN.md update into the same PR as the code change it documents where practical, rather than a same-day companion PR.
...
- Jira: CFL-90, CFL-91, CFL-93, CFL-94 all moved Backlog → In Progress → Code Review as their branches/PRs progressed (each transitioned explicitly, not inferred from a sibling). CFL-92 (their parent Epic) sat in Backlog despite this until corrected to In Progress on 2026-07-31 — see the Approved Operating Decisions entry below.
- All three PRs are now merged (2026-07-31, in order #56 → #57 → #58). **Jira status for CFL-90/91/93/94 not yet moved past Code Review as of this doc update** — needs an explicit transition to QA/Done, not assumed here; see the standing rule at the top of this file about not skipping child-item status updates.
- Next required action: transition CFL-90/91/93/94 in Jira to match the merge; CFL-95's PR #63 needed two rebases against these landing (once for #56, once for the combined #57+#58) since all touch `Calculator.jsx`/`calculator.css` — both used the same verified conflict resolution.
...
- Current work: v0.6.0 and CFL-30 Done. CFL-33/CFL-88 confirmed Done live. **CFL-90 (#56), CFL-91 (#57), CFL-93/94 (#58), and the icon source PR (#62) are all merged to `main`.** CFL-28's submission log is merged (#60); still needs a second-teammate review before Jira Done. CFL-95 (#63) is the last piece still open — code-complete, Code Review, pending Eldad's approval and joint design sign-off. It has been rebased twice already (once for #56, once for the combined #57+#58 landing) since both touch `Calculator.jsx`/`calculator.css`; each rebase used an identical, already-verified conflict resolution (kept CFL-95's header/`calculator-frame` structure, layered in `loggedDispatch`/`CookieBanner`/`TelemetryExport` from the telemetry PRs, dropped the superseded `.calculator-log-export*` CSS in favor of CFL-95's icon-button styles).
- Full integration (all of #56/#57/#58/#62/#63 together) was dry-run tested locally before any of the real merges, via a scratch branch never pushed anywhere: 33/33 QA checks passed against a production build, covering every CFL-90/91/93/94/95 acceptance criterion plus full regression. The real merges since then have matched that result exactly.
- Branch cleanup 2026-07-31: 39 stale branches deleted (33 from already-merged historical PRs, the 4 merged this session, and 2 from PRs closed as superseded — #59, #61). Only `main`, `feature/CFL-95-calculator-design-tweaks` (#63), and this doc branch remain.
...
- Next safe action: await Eldad's approval on PR #63 (currently commented, not yet approved, at its latest rebased commit); second-teammate review of CFL-28's submission log.
