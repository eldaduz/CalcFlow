...
- Human owner: Gavi
- AI used: Claude
- Note: this update lands late relative to the work it describes — the AI built and opened PR #56/#57/#58 without touching this file first, against the Mandatory AI Milestone Updates list below. Caught when Gavi asked directly. Treat "update the handoff" as part of finishing a Feature, not an optional follow-up.

## Current Release

Two tracks are in flight in parallel, one per owner:

- **Gavi — v0.5.0 (History and Memory): shipped, Done, tagged and released** (GitHub tag/release `v0.5.0`). **v0.6.0 — Complete User Experience is fully Done**: CFL-24, CFL-25, and **CFL-26 (Accessibility: CFL-73/74) all Done**, merged, deployed, and production-smoke-tested (PR #41, #46, #48). **CFL-30/CFL-28/CFL-33 (the last three original Gavi Features toward v1.0.0):** CFL-30 (Vercel Deployment: CFL-81/82) is **Done**, smoke-tested. CFL-33/CFL-87 (license report generation) is **Done**; CFL-33 itself and CFL-88 (second-teammate license review) remain Code Review. CFL-28 (Log Export) remains Code Review, still missing its actual committed `logs/calcflow-submission-log.json`. Epics CFL-5 (Calculator Experience) corrected to **Done**; CFL-7 (CI/CD) corrected to **In Progress** (both were stale — see Approved Operating Decisions).
- **Gavi — new Epic CFL-92 (Novelty and Easter Eggs) plus CFL-90/91 (extending CFL-6/CFL-28's logging scope):** all four items (CFL-90, CFL-91, CFL-93, CFL-94) are code-complete and in **Code Review**, three open PRs — [PR #56](https://github.com/eldaduz/CalcFlow/pull/56) (CFL-90), [PR #57](https://github.com/eldaduz/CalcFlow/pull/57, stacked on #56) (CFL-91), [PR #58](https://github.com/eldaduz/CalcFlow/pull/58) (CFL-93/94). See the dedicated entries below.
- **Eldad — v1.0.0 (Stable Final Release): in progress.** CFL-31 — Release Management tooling merged via PR #34, Ready for Deployment (the actual tag/release execution is his remaining step, and per the release sequence comes after Gavi's remaining Features finish). CFL-32 (Dependency Governance: CFL-85/86) and CFL-89 (Angle mode preservation bug) are both **Done**.

## Current Approved Sequence

1. Gavi: awaiting Eldad's review on PR #56/#57/#58 (CFL-90/91/93/94), and awaiting CFL-28's and CFL-33/CFL-88's own required second-teammate log/license reviews before those can reach Jira Done
...
- **CFL-93/CFL-94 (Cookie Consent Parody):** new `CookieBanner.jsx` — fixed-position banner (**second scoped exception** to `design.md`'s "no fixed-position layout" rule, same pattern as the CFL-24/70 shortcuts panel, now written into `design.md`'s Decisions section rather than resting on Eldad's earlier verbal sign-off), single "Accept" button, no other option, dismissing sets no state/flag anywhere. Not persisted across mounts — reappears every time, deliberately (the joke is that a real cookie banner wouldn't). Independent of CFL-90/91, branched straight off `main`. Verification: clean pipeline, 265 tests (4 new), 97.64% coverage, real-browser check confirming fixed positioning, single button, non-blocking interaction with the calculator underneath, clean dismiss, zero console errors. PR: [PR #58](https://github.com/eldaduz/CalcFlow/pull/58), Eldad requested, Code Review.
- Jira: CFL-90, CFL-91, CFL-93, CFL-94 all moved Backlog → In Progress → Code Review as their branches/PRs progressed (each transitioned explicitly, not inferred from a sibling).
- Next required action: none from Gavi's side. Awaiting Eldad's review on all three PRs. When #56 merges, rebase #57 onto `main` before it can merge cleanly (squash-merge will otherwise show `mergeable_state: dirty` per the project's known stacked-branch gotcha — see `CLAUDE_PROJECT_NOTES.md` §5).
...
- **Epics drift silently, same as Features/Stories — checked and corrected 2026-07-30:** CFL-5 ("Calculator Experience") sat in Backlog despite all five children (CFL-22/23/24/25/26) being Done — corrected to Done. CFL-7 ("Continuous Integration and Deployment") sat in "Selected for Development" despite two of three children (CFL-29, CFL-30) being Done and the third (CFL-31) at Ready for Deployment — corrected to In Progress (can't go Done until CFL-31 does). This is Eldad's Epic; corrected based on live cross-checking of its children, not assumed on his behalf.
...
- Current work: v0.6.0 is fully Done. Of the original three v1.0.0 Features, CFL-30 is Done; CFL-28 and CFL-33/CFL-88 remain Code Review pending their own required second-teammate reviews. New Novelty Telemetry work (CFL-90/91/93/94, Epic CFL-92) is code-complete, three PRs open (#56/#57/#58), all in Code Review awaiting Eldad. Nothing left to build on Gavi's side right now — everything open is waiting on review, not more code.
- PR #52 vs #53 (the competing docs-PR conflict flagged in the prior handoff) is resolved: Eldad closed #52 without merging and approved the corrected #53, which is merged. CFL-32/CFL-89 confirmed Done live. `PROJECT_PLAN.md`/`design.md` scope additions for the telemetry work (PR #54) are also merged.
- Current risk: **PR #57 (CFL-91) is stacked on PR #56 (CFL-90)** — if anything else merges to `main` before #56, or if #56 merges via squash before #57 rebases, #57 will show as `dirty`/conflicted per the project's known squash-merge-breaks-stacked-branches gotcha. Not yet hit, just flagged in advance.
- Also flagged, not fixed: a StrictMode-only dev-mode duplicate-logging quirk found while QA'ing CFL-91 (see the CFL-90/91/93/94 entry above) — confirmed absent from production builds, Eldad's CFL-16/27 territory, no Bug filed since it has zero production impact.
- Next safe action: await Eldad's review on PR #56/#57/#58; await his (or another second reviewer's) log/license review to close out CFL-28 and CFL-33/CFL-88; watch the #56→#57 rebase dependency once #56 merges.
