...
- Human owner: Gavi
- AI used: Claude
- **v1.0.0 is Done.** CFL-31 (verified live via Jira and GitHub): Done, `fixVersion` "v1.0.0 — Stable Final Release" released 2026-08-02. Tag/GitHub Release `v1.0.0` confirmed present. Live JQL query (`project = CFL AND status not in (Done)`) returned **zero** open issues at time of writing — this corrects this file's own previously-stale "CFL-31 still pending" wording (§"Latest Handoff" below), a live Jira/GitHub-vs-doc mismatch caught during [PR #69](https://github.com/eldaduz/CalcFlow/pull/69)'s mandatory pre-work verification.
- [PR #69](https://github.com/eldaduz/CalcFlow/pull/69) (untracked repo-hygiene, not its own Jira item, same convention as PR #66's fix): moved `PROJECT_PLAN.md`/`SECOND_BRAIN.md`/`CLAUDE_PROJECT_NOTES.md`/`CODEX_HANDSHAKE.md`/`CODEX_MODEL_GUIDE.md` into `docs/agents/` (kept `CLAUDE.md`/`AGENTS.md` at root — both are auto-loaded by their respective tools only from repo root); rewrote `README.md` as a production-style doc with a live screenshot; settled `package.json`'s stale, never-deliberate `"ISC"` claim to `"UNLICENSED"` (all rights reserved) since the repo is public but no reuse grant was ever intended. Opened, Eldad requested as reviewer. Since all Jira work is Done, this is genuinely trailing cleanup, not blocking anything.

## Current Release

- **Gavi — v0.6.0 (Complete User Experience) is fully Done**: CFL-24, CFL-25, CFL-26 (CFL-73/74) all Done, merged, deployed, smoke-tested. Of the original three v1.0.0 Features: **CFL-30 (Vercel Deployment) is Done.** **CFL-33 (License Reporting) and CFL-88 are Done, verified live** — Eldad's PR #50 review counted as the required second-teammate license sign-off.
- **Gavi — Novelty Telemetry (Epic CFL-92, plus CFL-90/91 under CFL-6/CFL-28): all Done.** CFL-90, CFL-91, CFL-93, CFL-94, and their parent Epic CFL-92 are all **merged and Jira Done** — [PR #56](https://github.com/eldaduz/CalcFlow/pull/56) (CFL-90), [PR #57](https://github.com/eldaduz/CalcFlow/pull/57) (CFL-91), [PR #58](https://github.com/eldaduz/CalcFlow/pull/58) (CFL-93/94). Verified via a dry-run QA pass (33/33 checks, run against a local scratch merge before any real merges landed) plus a live production smoke test (9/9 checks, https://calc-flow-fawn.vercel.app/), both confirmed by Gavi. See the dedicated entry below for full scope detail.
- **Gavi — CFL-95 (Calculator Design Tweaks): Done, merged.** [PR #63](https://github.com/eldaduz/CalcFlow/pull/63) merged 2026-07-31, including the two follow-up fixes (memory indicator hidden in Basic mode; History panel spans the calculator's full height on desktop) and one real review round (2 findings fixed, 2 flagged as not reproducing against the actual commit, resolved by Eldad's final approval). Joint design sign-off satisfied via that approval — no longer pending in design.md. Post-merge QA (33/33 checks against the real merged `main` + production build) and a live production smoke test both passed, confirmed by Gavi.
- **Gavi — CFL-28 (Log Export and Submission Evidence): Done.** Its `logs/calcflow-submission-log.json` is exported and committed ([PR #60](https://github.com/eldaduz/CalcFlow/pull/60), merged). Resolved 2026-08-02: PR #60 contained only that log file and this doc, no application code — Eldad's review explicitly audited the log content (event types/count, production-build provenance) and stated it satisfies CFL-28's second-teammate review requirement. That's the dedicated review the AC asks for, not an incidental code approval. **CFL-78** (its child Task) and parent Epic **CFL-6** (no other children) both moved to Done alongside it.
- **Gavi — Export control consolidation (2026-07-31, follow-up, not its own Jira item):** after CFL-95 moved Export Logs into a header icon, CFL-91's separate "Export Telemetry" button was left as a stray plain-text control below the calculator, and both export status messages were found to never auto-clear. [PR #66](https://github.com/eldaduz/CalcFlow/pull/66) consolidates both exports into one header icon with a small menu, and fixes the status-message bug — logged as comments on CFL-91 and CFL-28 rather than a new Jira Bug, per Gavi's explicit call. **Merged 2026-08-02, unintentionally** (Eldad meant to approve only; QA was meant to happen before merge), then post-merge QA'd and Production-smoke-tested successfully — see the dedicated PR #66 entry below. Fully resolved; no longer blocking anything.
...
1. Eldad: CFL-31 (execute the v1.0.0 tag and GitHub Release) — the last remaining item toward v1.0.0. Everything else in scope (CFL-27/28/29/30/32/33 and their children) is Done.

## PR consolidation (2026-07-31, resolved)

A three-way pileup briefly existed on this exact file — [PR #59](https://github.com/eldaduz/CalcFlow/pull/59), [PR #60](https://github.com/eldaduz/CalcFlow/pull/60), and [PR #61](https://github.com/eldaduz/CalcFlow/pull/61) were all touching it at once, none merged, none reviewed. Resolved by folding #59's and #61's accurate content into the PR #64 revision (both closed as superseded) and letting #60 (real evidence-file content, not just docs) merge on its own. Same-file collisions across simultaneous docs PRs happened twice this session (see the PR #52 vs #53 incident below) — the recurring cause is landing a docs update in its own PR that outlives its subject PR. This update itself follows the corrected practice: folded into PR #66 (the code change it documents) instead of a new companion PR.
...
### Gavi — CFL-90 / CFL-91 / CFL-93 / CFL-94 — Novelty Telemetry (2026-07-30)

- **New scope, approved by Gavi, concept pre-approved by Eldad:** a deliberately comedic implementation of the logging/tracing/metrics capabilities the project already demonstrates seriously — see `PROJECT_PLAN.md` §2's new in-scope bullet and §17's carve-out. New Product Epic **CFL-92 (Novelty and Easter Eggs)** for the cookie banner; CFL-90/91 are Stories under the existing CFL-28 Feature (logs are logs, one export, per Gavi's explicit direction — not a separate module or separate log file). Groundwork (Jira items, `PROJECT_PLAN.md`/`design.md` scope) landed first in [PR #54](https://github.com/eldaduz/CalcFlow/pull/54), merged.
- **CFL-90 (Log Button Presses and Trace Spans):** extends `logger.js`'s existing `logEvent`/shared store with `logButtonPress()` — every button (keypad, scientific, memory, mode toggle, keyboard-only shortcuts panel) gets a fabricated deadpan-APM-styled trace span (`traceId`, `spanId`, `durationNs`); `=` additionally gets a 4-span nested waterfall (`parse-expression`/`evaluate`/`format-result`/`render`). `BUTTON_PRESS` entries use a rolling 6-hour eviction window; the three real calculation event types are untouched, never evicted. `Export Logs` (CFL-28) already returns the whole shared store, so click/trace data rides along with real events in one export — no export-path change needed. Verification: clean pipeline, 266 tests (8 new), 96.9% coverage, real-browser Playwright check confirming spans/waterfall/mode-toggle/shortcuts logging with zero console errors. PR: [PR #56](https://github.com/eldaduz/CalcFlow/pull/56), reviewed and approved by Eldad, **merged** to `main`.
- **CFL-91 (Compute and Export Ridiculous Usage Metrics):** adds `getTelemetryMetrics()`/`getButtonPressLogs()` to `logger.js` (most-pressed button, most common calculation error, a neglected button, total presses, longest streak without `AC`) and (originally) a `TelemetryExport.jsx` component — a **separate** "Export Telemetry" button/status downloading button-press entries plus the metrics summary as its own file, never touching `logs/calcflow-submission-log.json`. No always-visible panel, export-on-demand only, per product decision. Originally stacked on CFL-90's branch; retargeted to `main` and rebased once #56 merged, which dismissed the first approval — Eldad re-reviewed and approved the rebased commit (same diff, new base). Verification: clean pipeline, 276 tests (10 new), 97.14% coverage, real-browser check (dev + production preview builds) confirming both exports stay cleanly separated. PR: [PR #57](https://github.com/eldaduz/CalcFlow/pull/57), reviewed and approved by Eldad, **merged** to `main`. **`TelemetryExport.jsx`'s standalone button was later folded into `ExportMenu.jsx` (PR #66, 2026-07-31)** — see that PR's entry below; the export logic and file format are unchanged, only the trigger UI moved.
- **Found during CFL-91 QA, not fixed, no Bug filed:** `CALCULATION_ERROR` logs twice for one `=` press under React StrictMode in `npm run dev` (pre-existing — `logEvent()` is called as a side effect directly inside the reducer body in `evaluateCurrentExpression`, which StrictMode intentionally double-invokes in dev to catch impure reducers). Confirmed via a production preview build (`npm run preview`) that it logs exactly once there — StrictMode never runs in production, so this never reaches the real submission log or an actual user. Pre-existing CFL-16/CFL-27 territory (Eldad's), not introduced by CFL-90/91 and not in their scope to fix; flagging here rather than silently working around it.
- **CFL-93/CFL-94 (Cookie Consent Parody):** new `CookieBanner.jsx` — fixed-position banner (**second scoped exception** to `design.md`'s "no fixed-position layout" rule, same pattern as the CFL-24/70 shortcuts panel, now written into `design.md`'s Decisions section rather than resting on Eldad's earlier verbal sign-off), single "Accept" button, no other option, dismissing sets no state/flag anywhere. Not persisted across mounts — reappears every time, deliberately (the joke is that a real cookie banner wouldn't). Independent of CFL-90/91, branched straight off `main`. Verification: clean pipeline, 265 tests (4 new), 97.64% coverage, real-browser check confirming fixed positioning, single button, non-blocking interaction with the calculator underneath, clean dismiss, zero console errors. PR: [PR #58](https://github.com/eldaduz/CalcFlow/pull/58), reviewed and approved by Eldad, **merged** to `main`.
- Jira: CFL-90, CFL-91, CFL-93, CFL-94 all moved Backlog → In Progress → Code Review → QA → Ready for Deployment → **Done** (each transitioned explicitly, not inferred from a sibling). CFL-92 (their parent Epic) sat in Backlog despite this until corrected to In Progress, then moved to **Done** once both children (CFL-93, CFL-94) completed — Epics don't auto-roll-up, corrected explicitly both times.
- All three PRs merged 2026-07-31, in order #56 → #57 → #58. Verified via a dry-run QA pass (33/33 checks, local scratch merge before any real merges) and a live production smoke test (9/9 checks, https://calc-flow-fawn.vercel.app/), both confirmed by Gavi. Done-note evidence comments added to each Jira issue.
- CFL-95's PR #63 needed two rebases against these landing (once for #56, once for the combined #57+#58) since all touch `Calculator.jsx`/`calculator.css` — both used the same verified conflict resolution, confirmed identical when applied for real.
- Next required action: none from Gavi's side on CFL-90/91/92/93/94 — all Done. See the CFL-28/CFL-78 entry and the PR #66 entry below for what's still open.
...
- **Epics drift silently, same as Features/Stories — checked and corrected 2026-07-30, recurred and corrected again 2026-07-31:** CFL-5 ("Calculator Experience") sat in Backlog despite all five children (CFL-22/23/24/25/26) being Done — corrected to Done. CFL-7 ("Continuous Integration and Deployment") sat in "Selected for Development" despite two of three children (CFL-29, CFL-30) being Done and the third (CFL-31) at Ready for Deployment — corrected to In Progress. CFL-92 ("Novelty and Easter Eggs") sat in Backlog despite both children (CFL-93, CFL-94) being in Code Review for a day — corrected to In Progress. Same root cause every time: an Epic's own status field is a separate field that nothing auto-updates when a child moves — check it explicitly at every milestone, don't assume from a child's status.
- **Novelty telemetry (CFL-90/91/92/93/94) is real in-scope work, not a throwaway joke** — it's a comedic implementation of the logging/tracing/metrics capabilities this project is meant to demonstrate seriously elsewhere; alerting/monitoring stays explicitly out of scope even in that spirit. See `PROJECT_PLAN.md` §2/§17.
- **Logs are logs — CFL-90's click/trace data shares CFL-28's existing logger/export, not a separate module or file** (Gavi's explicit direction, 2026-07-30). CFL-91's computed metrics are the one thing kept in a genuinely separate export (`Export Telemetry`), since they're derived, not raw log data.
...
## CFL-95 — Calculator Design Tweaks (Done, merged, joint sign-off complete)

- Jira: CFL-95, **Done**. Six related layout changes reviewed as an interactive Artifact mockup with Gavi across 11+ rounds (desktop and mobile) before any code changed: https://claude.ai/code/artifact/9c378fcb-4cbe-47e6-a59c-1bf49081210d
- **Joint sign-off completed 2026-07-31** via Eldad's approval of PR #63 at its final commit — design.md updated to reflect this, no longer "pending." Same design-control rule as CFL-25's base-keypad decision, satisfied the same way (approval on the implementing PR after one real review round).
...
- **Follow-up fixes (2026-07-31, same PR):** memory indicator no longer shows in Basic mode (memory is only reachable via `MC`/`MR`/`M+`/`M-`, which live exclusively in the Scientific grid); desktop History panel now spans the calculator's full height instead of only its content height, with its entry list scrolling internally. `design.md` updated to match. All 266 tests still pass; re-verified in a real browser before pushing.
- **Review round (2026-07-31):** Eldad requested changes with 4 findings. 2 were real and fixed: `Display.jsx`'s top-right slot now only takes `role="alert"`/`aria-live="assertive"` when actually showing an error, not for ordinary previous-expression updates; `History.jsx`'s Clear button only renders once there's at least one entry. 2 (claims that `SET_MODE` and mode-preservation-on-`CLEAR` were missing from the reducer) did not reproduce against the actual commit under review — verified directly via `git show <sha>:src/lib/expressionEngine.js`, both cases were already correctly handled. Replied with the evidence; Eldad's subsequent approval covered the two real fixes.
- Structural risk realized twice, both resolved identically: branched off `main`, not stacked on CFL-90/91/93/94's PRs (#56/#57/#58), both touching `Calculator.jsx`/`calculator.css`. Required a rebase-and-resolve after #56 merged, then again after #57+#58 merged (each rebase dismissed Eldad's prior approval, requiring a fresh one — flagged explicitly on the PR each time). Both resolutions used the same conflict pattern: keep CFL-95's header/`calculator-frame` structure, layer in `loggedDispatch`/`CookieBanner`/`TelemetryExport` from the telemetry PRs, drop the superseded `.calculator-log-export*` CSS in favor of CFL-95's icon-button styles.
- PR: [PR #63](https://github.com/eldaduz/CalcFlow/pull/63), reviewed and approved by Eldad, **merged** to `main` 2026-07-31.
- Verification after merge: post-merge QA (33/33 checks against the real merged `main` + production build, re-run since the pre-merge dry run predated the review-round fixes) and a live production smoke test (calculator-frame/icon-buttons/slider/Scientific-mode markers all present, zero console errors) both passed, confirmed by Gavi.
- Follow-up discovered after merge, not part of this issue's own scope: the Export icon vs. the separate Export Telemetry button looked like a stray leftover control once Export Logs became a header icon — see the "Export control consolidation" entry above and PR #66.
- Next safe action: none. Fully Done.

## PR #66 — Export control consolidation (2026-07-31, follow-up to CFL-95/CFL-91)

- Not its own Jira item — logged as comments on CFL-91 and CFL-28 instead, per Gavi's explicit instruction ("log in Jira as a bug" was considered and declined; this shipped fixed in the same PR that found it, not worth a separate ticket for something this minor).
- Trigger: after CFL-95 merged, Gavi spotted the header's single Export Logs icon sitting next to a leftover plain-text "Export Telemetry" button below the calculator (CFL-91's control, never restyled by CFL-95's redesign since it was out of that Feature's scope) — looked like a stray leftover control.
- Considered merging the two exports into one file first (Gavi's hypothesis: telemetry might be a superset of the real log). Checked the actual data: it isn't — the real log interleaves `BUTTON_PRESS` with `CALCULATION_SUCCESS`/`CALCULATION_ERROR` evidence events that telemetry omits entirely, and telemetry adds computed `metrics` the real log doesn't have. Neither is a superset; merging into one file would have dropped real evidence data. Kept them as two distinct files.
- Resolution: new `ExportMenu.jsx` replaces `LogExport.jsx` + `TelemetryExport.jsx` (both deleted) — one header icon, `aria-haspopup`/`aria-expanded`, opens a small `role="menu"` with two `role="menuitem"` options. Each triggers the same underlying export logic as before (same files, same data), just from one shared trigger.
- Also fixed in the same pass: both export status messages ("Exported N entries.") never auto-cleared, staying on screen indefinitely once triggered — a real bug in both original components, not new to this consolidation. Now clears after 4 seconds (`setTimeout`, matching the existing `vi.useFakeTimers()` convention already used in `tests/logger.test.js`).
- **Review round 1 (Eldad, 2026-07-31, Changes Requested):** the status-clear `setTimeout` from the fix above was itself buggy — `showStatus` scheduled a new timeout on every call without cancelling the prior one, so exporting Logs then Telemetry within 4 seconds let the first timeout clear the second message early. Eldad asked for the timeout ID to be stored in a ref, cleared before scheduling a replacement and on unmount, plus a fake-timer regression covering two exports before the first delay expires.
- **Fixed (2026-07-31/08-02):** `ExportMenu.jsx` now tracks the pending timeout in a `useRef`, clears it at the start of every `showStatus` call and in a `useEffect` cleanup on unmount. Added the requested regression test (`exportMenu.test.jsx`: export Logs, advance 2s, export Telemetry, confirm the telemetry status survives its own full 4s window rather than being cut short by the first timeout). Full pipeline re-verified: clean `npm ci`, lint, format:check, 288 tests (289 total repo-wide with this addition — was 287), 97.21% statement coverage, production build, `git diff --check` all clean.
- Verification: clean pipeline (lint, format, 288 tests — `exportMenu.test.jsx` now includes the timeout-cancellation regression), production build, real-browser check (single icon confirmed, both exports still produce their distinct correct files, menu closes after selection, status auto-clears correctly even with two exports in quick succession, zero console errors).
- PR: [PR #66](https://github.com/eldaduz/CalcFlow/pull/66), Eldad requested, two Changes Requested rounds (the second, at commit `a7fd05e`, reconfirming the same blocking issue was still unresolved at that head). Fix pushed 2026-08-02 addressing both rounds.
- **Merge (2026-08-02, unintentional):** Eldad approved the fix at the correct head commit (`975fa62`, `commit_id` matches) and CI passed — then merged the PR. Per Eldad, the merge itself wasn't meant to happen yet; the intent was to approve and let QA run first. Since a merge can't be undone, he corrected CFL-28's Jira status (which had briefly auto-followed to Done) back to Ready for Deployment and flagged it to Gavi rather than leaving it misrepresented. Merge commit: `2ddbab7`.
- **Post-merge QA (2026-08-02, Gavi/Claude), against `main`@`2ddbab7`:**
  - Automated: clean `npm ci`, lint, format:check, 288/288 tests, 97.21% statement coverage, clean build, `git diff --check` clean — matches the PR's own evidence.
  - Manual (real browser, Playwright against dev server): single Export header icon opens a menu with Export Logs / Export Telemetry; both produce distinct correct files; menu closes on selection; zero console errors. Regression: basic arithmetic, History recording, Scientific mode toggle all unaffected.
  - **Timeout-cancellation fix verified precisely**, not just by code inspection: exported Logs, then Telemetry ~2s later; at t=4.25s (past the _first_ export's original 4s clear timer) the Telemetry status message was still showing — confirms the old timer was cancelled rather than wiping the newer message early, exactly the bug Eldad's two review rounds caught.
  - Evidence logged as comments on CFL-28 and CFL-91 (2026-08-02), matching the "not its own Jira item" convention established above.
- **Deploy gap found, then resolved (2026-08-02):** Production initially still served the pre-#66 build (`last-modified: 2026-07-31T15:04:44Z`) with zero deployment attempts recorded for `2ddbab7` in GitHub's deployment API. Eldad investigated via the Vercel dashboard and resolved it. Re-verified: https://calc-flow-fawn.vercel.app/ now serves `2ddbab7` (`last-modified: 2026-08-02T16:09:14Z`). Root cause of the original gap not recorded here — ask Eldad if it recurs.
- **Production smoke test passed (2026-08-02, Gavi/Claude):** consolidated Export icon+menu live, basic arithmetic correct (7×6=42), Export Logs downloads correctly from production, zero console errors.
- Next safe action: none — this episode is fully closed, including the CFL-28/CFL-78 second-teammate-review question (see below).

## Latest Handoff

- Current work: v0.6.0, CFL-30, and all of CFL-90/91/92/93/94/95 are **Done**, merged, and verified live in production (dry-run QA + post-merge QA + production smoke test, all confirmed by Gavi). CFL-33/CFL-88 confirmed Done live. **PR #66 merged 2026-08-02 (unintentionally, per Eldad — see its dedicated entry above), passed post-merge QA on `main`, and — after an initial deploy gap Eldad resolved — is now live and smoke-tested in Production.** **CFL-28/CFL-78 resolved to Done 2026-08-02**: Eldad's PR #60 review explicitly audited the log file's content and stated it satisfies the second-teammate review AC — confirmed by Gavi, not assumed. Parent Epic CFL-6 (no other children) moved to Done alongside them.
- Branch cleanup, two passes 2026-07-31: 39 stale branches deleted in the first pass (33 from already-merged historical PRs, 4 merged this session, 2 from PRs closed as superseded — #59, #61), then 4 more (3 local, 1 remote) after #60/#63/#64 merged. Only `main` and the currently-active PR branch remain at any given time — check `git branch --list` and `gh`/API branch list before assuming this stays true. `fix/export-menu-consolidation` (PR #66's source branch) deleted 2026-08-02 now that the episode resolved cleanly.
...
- **v1.0.0 status: fully Done, including CFL-31.** CFL-27/28/29/30/31/32/33 (and their children) all Done, verified live — CFL-31's tag/GitHub Release execution (the item this section previously listed as the last remaining step) is confirmed complete: `v1.0.0` tagged and released 2026-08-02. Corrected here after the live-state check in [PR #69](https://github.com/eldaduz/CalcFlow/pull/69) found this section stale.
- Next safe action: none outstanding in Jira (zero open CFL issues as of 2026-08-02). [PR #69](https://github.com/eldaduz/CalcFlow/pull/69) is trailing repo-hygiene work awaiting Eldad's review; nothing else pending.
